...
Page {
    ...
    // ╚══════════════════════════════════════════════════════════════════╝
    readonly property real headerHeight:    6.7   // *** HEADER HEIGHT *** change this number
    readonly property real texOpacityDark:  0.1 // ← dark  mode texture intensity
    readonly property real texOpacityLight: 0.06  // ← light mode texture intensity
    ...
    property bool   pdfSorryVisible:  false

    // ── Hero banner ───────────────────────────────────────────────────────────
    property bool heroCollapsed: false

    // ── Book long-press context menu ──────────────────────────────────────────
    property var  _longPressBook:  null   // book object that triggered the press
    property bool bookMenuVisible: false  // controls the bottom-sheet visibility
    // Haptics — created dynamically so the file loads even if QtFeedback is absent
    property var  _hapticEffect:   null
    ...
    header: PageHeader {
        ...
        contents: Item {
            ...
            // RIGHT: lists/filter icon — leftmost of right buttons
            Rectangle {
                id: headerListsBtn
                anchors { right: headerBookBtn.left; rightMargin: units.gu(0.2)
                ...
                Icon {
                    ...
                    color: libraryPage.currentFilter !== "all"
                           ? "#4CAF50"
                           : (root.isDarkMode ? "#666666" : "#999999")
                }
                MouseArea {
                    anchors.fill: parent
                    onClicked: {
                        Library.init()
                        libraryPage._filterLists = Library.getLists()
                        libraryPage.filterMenuVisible = true
                    }
                }
            }
            ...
            Rectangle {
                ...
                MouseArea {
                    anchors.fill: parent
                    onClicked: libraryPage.addDialogVisible = true
                }
            }
            ...
            Column {
                ...
                // "B" = Ubuntu Light,  "earthen" = Ubuntu Medium — subtle contrast
                Row {
                    spacing: 0
                    Label {
                        text: "B"
                ...
                // "My" = Ubuntu Medium,  "Library" = Ubuntu Light
                Row {
                    spacing: units.dp(4)
                    Label {
                        text: "My"
    ...
    property string currentFilter: "all"   // "all" | "unread" | "in_progress" | list-id
    property bool   filterMenuVisible: false
    property var    _filterLists:  []

    function refresh() {
        Library.init()
        _allBooks = Library.getBooks()
        var f = libraryPage.currentFilter
        if (f === "all") {
            books = _allBooks
        } else if (f === "unread") {
            books = _allBooks.filter(function(b) { return b.read_percent === 0 && !b.is_finished })
        } else if (f === "in_progress") {
            books = _allBooks.filter(function(b) { return b.read_percent > 0 && !b.is_finished })
        } else {
            books = Library.getBooksInList(f)
        }
        console.log("LibraryPage: refreshed, filter:", f, "books:", books.length)
    }

    Component.onCompleted: {
        refresh()
        // Try to create a HapticsEffect for long-press feedback.
        // Uses Qt.createQmlObject so the page loads even if QtFeedback is absent.
        try {
            libraryPage._hapticEffect = Qt.createQmlObject(
                'import QtFeedback 5.0; HapticsEffect { duration: 25; intensity: 0.9 }',
                libraryPage)
        } catch(e) {
            console.log("Haptics: QtFeedback 5.0 not available —", e)
        }
    }
    onVisibleChanged: { if (visible) { refresh(); libraryPage.heroCollapsed = false } }
            ...
            // a late-arriving META signal (if any) can still update the record.
                ...
                if (cIdx === cTotal - 1) {
                    ...
                    if (cBid && cFull.length > 50) {
                        libraryPage._writeCoverFile(cBid, cFull)
                        libraryPage.refresh()
                    }
                } else {
    ...
    // ── Off-screen Image used to save cover JPEG to disk ─────────────────────
    // Qt's Image element decodes data: URIs natively. grabToImage().saveToFile()
    // is the standard QML way to persist image data — no manual base64 decoding.
    property string _coverSaveId: ""
    ...
    Image {
        ...
        onStatusChanged: {
            if (status !== Image.Ready || libraryPage._coverSaveId === "") return
            var bid  = libraryPage._coverSaveId
            var path = Library.coverPath(bid)
            libraryPage._coverSaveId = ""   // guard against re-entry
            // Pass the actual image dimensions so grabToImage renders at full size.
            // IMPORTANT: clear source only INSIDE the callback — clearing it first
            // would unload the backing pixels before the grab completes, producing
            // a solid-black result.
            var sw = sourceSize.width, sh = sourceSize.height
            grabToImage(function(result) {
                source = ""   // release backing data after capture
                if (result && result.saveToFile(path)) {
                    Library.init()
                    Library.updateCoverLocal(bid, path)
                    console.log("Cover: saved →", path)
                    libraryPage.refresh()
                } else {
                    console.log("Cover: saveToFile failed for", bid)
                }
            }, Qt.size(sw, sh))
        }
    }
                    ...
                    if (st === ContentTransfer.Charged) {
                        ...
                        if (items && items.length > 0) {
                            ...
                            if (fname.toLowerCase().slice(-5) === ".epub") {
                                ...
                                // NOTE: do NOT pass a trailing slash — ContentHub
                                // appends its own "/" separator, so a trailing slash
                                // creates a double "//" path that makes both move and
                                // copy fail.
                                var destDir  = "/home/phablet/.local/share/bearthen.russs95"
                                var destPath = destDir + "/" + fname
                                var destUrl  = "file://" + destPath
                                var moved      = false
                                var fileSizeKb = 0
                                try {
                                    moved = items[0].move(destDir)
                                } catch(moveErr) {
                                    console.log("EPUB move() threw:", moveErr)
                                }
                                if (!moved) {
                                    // Fallback: copy the file ourselves via XHR.
                                    // This works even when ContentHub move/copy fails
                                    // (e.g. AppArmor policy differences between processes).
                                    try {
                                        var rdXhr = new XMLHttpRequest()
                                        rdXhr.open("GET", url, false)
                                        rdXhr.responseType = "arraybuffer"
                                        rdXhr.send()
                                        if (rdXhr.response) {
                                            fileSizeKb = Math.round(rdXhr.response.byteLength / 1024)
                                            var wrXhr = new XMLHttpRequest()
                                            wrXhr.open("PUT", destUrl, false)
                                            wrXhr.send(rdXhr.response)
                                            moved = true
                                            console.log("EPUB XHR copy: OK →", destPath,
                                                        "(" + fileSizeKb + " KB)")
                                        }
                                    } catch(cpErr) {
                                        console.log("EPUB XHR copy threw:", cpErr)
                                    }
                                }
                                // For move() path: try a HEAD request for file size
                                if (moved && fileSizeKb === 0) {
                                    try {
                                        var szXhr = new XMLHttpRequest()
                                        szXhr.open("HEAD", destUrl, false)
                                        szXhr.send()
                                        var cLen = parseInt(szXhr.getResponseHeader("Content-Length") || "0")
                                        if (cLen > 0) fileSizeKb = Math.round(cLen / 1024)
                                    } catch(e) {}
                                }
                                var finalUrl = moved ? destUrl : url
                                console.log("EPUB:", moved ? "saved → " + finalUrl : "FAILED, using staging: " + url)
                                libraryPage._importEpub(finalUrl, fname, fileSizeKb)
                            } else {
    ...
    // ── Write cover JPEG to disk ──────────────────────────────────────────────
    // Hands the data: URI to Qt's own Image decoder (no manual base64 work),
    // then uses grabToImage().saveToFile() to persist as a real JPEG file.
    function _writeCoverFile(bookId, dataUrl) {
        libraryPage._coverSaveId = bookId
        _coverSaveImg.source     = dataUrl
    }

    // ── Import EPUB function ──────────────────────────────────────────────────
    function _importEpub(fileUrl, fileName, fileSizeKb) {
        ...
        // NOTE: filePath is the moved path under .local/share/bearthen.russs95/
    ...
    // ── Hero welcome banner ────────────────────────────────────────────────────
    // Greets the user above the grid. Collapses to 0 height when the up-fleche
    // is tapped. Re-expands automatically each time the page becomes visible.
    Rectangle {
        id: heroBanner
        anchors { top: pageHeader.bottom; left: parent.left; right: parent.right }
        height: (libraryPage.books.length === 0 || libraryPage.books.length >= 2 || libraryPage.heroCollapsed)
                ? 0 : heroColumn.implicitHeight + units.gu(3)
        ...
        Column {
            ...
            anchors {
                ...
                topMargin: units.gu(2); leftMargin: units.gu(2.5); rightMargin: units.gu(2.5)
            }
            spacing: units.gu(1.0)

            // Book icon above heading
            Icon {
                anchors.horizontalCenter: parent.horizontalCenter
                width: units.gu(5); height: units.gu(5)
                name: "stock_ebook"
            ...
            Label {
                ...
                color: root.isDarkMode ? "#88BB88" : "#2C5F2E"
            ...
            Label {
                anchors.horizontalCenter: parent.horizontalCenter
                width: parent.width
                text: "Upload your own EPUBs or discover and download new ones to build your library."
                font.pixelSize: units.gu(1.65); font.weight: Font.Light
                color: root.isDarkMode ? "#4A7A4A" : "#4A7A4A"
                horizontalAlignment: Text.AlignHCenter
                wrapMode: Text.WordWrap; lineHeight: 1.4
            }

            // Up-fleche — tap to collapse the hero and see the full grid
            Item {
                ...
                Icon {
                    ...
                    name: "go-up"
                    color: root.isDarkMode ? "#2C5F2E" : "#4A7A4A"
                }
                MouseArea {
                    anchors.fill: parent
                    onClicked: libraryPage.heroCollapsed = true
                }
            }
        }
    }
    ...
    GridView {
        id: bookGrid
        anchors {
            top: heroBanner.bottom
            left: parent.left; right: parent.right; bottom: parent.bottom
            leftMargin: units.gu(1.5)
            rightMargin: units.gu(1.5)
            bottomMargin: units.gu(1.5)
        }
        // topMargin as a Flickable content margin — adds space above the first row
        // of book cards WITHOUT pushing the clip boundary away from the green divider
        topMargin: units.gu(1.5)
        visible: libraryPage.books.length > 0
        clip: true
        cellWidth: width / 2        // exact half — cards centred via their inner 0.5gu margins
        cellHeight: units.gu(25)
        model: libraryPage.books

        // Pull down past the top to restore the hero banner
        onMovementEnded: {
            if (libraryPage.heroCollapsed && contentY < -units.gu(3))
                libraryPage.heroCollapsed = false
        }
        ...
        delegate: Item {
            ...
            Rectangle {
                ...
                MouseArea {
                    ...
                    onPressAndHold: {
                        libraryPage._longPressBook = modelData
                        libraryPage.bookMenuVisible = true
                        // Haptic pulse — gracefully skipped if QtFeedback unavailable
                        if (libraryPage._hapticEffect) {
                            try { libraryPage._hapticEffect.start() } catch(e) {}
                        }
                    }
                }
            }
    ...
    // ── "Add to your library" bottom-sheet dialog ─────────────────────────────
    // Scrim fades in/out; sheet slides up from below and back down on close.
    Rectangle {
        id: addDialogOverlay
        anchors.fill: parent
        color: "#88000000"
        opacity: libraryPage.addDialogVisible ? 1.0 : 0.0
        Behavior on opacity { NumberAnimation { duration: 200 } }
        // Keep interactive (visible=true) so the slide-out animation completes;
        // disable the dismiss hit-area while closing so stray taps don't re-open.
        visible: true
        z: 160

        MouseArea {
            anchors.fill: parent
            enabled: libraryPage.addDialogVisible
            onClicked: libraryPage.addDialogVisible = false
        }

        Rectangle {
            id: addSheet
            anchors { left: parent.left; right: parent.right }
            // 2.5 gu top margin inside sheet + 2 gu bottom padding = 4.5 gu total
            height: addSheetCol.implicitHeight + units.gu(4.5)
            // Slide up 7 gu above screen bottom when open; park off-screen when closed
            y: libraryPage.addDialogVisible
               ? parent.height - height - units.gu(7)
               : parent.height + units.gu(1)
            Behavior on y { NumberAnimation { duration: 280; easing.type: Easing.OutCubic } }
            color: root.isDarkMode ? "#1C1C1C" : "#FFFFFF"
            radius: units.dp(18)
            MouseArea { anchors.fill: parent; onClicked: {} }  // swallow stray taps
            ...
            Column {
                ...
                Label {
                    ...
                    color: root.isDarkMode ? "#888888" : "#AAAAAA"
                }
                ...
                ListItem {
                    height: units.gu(7.5)
                    color: "transparent"
                    divider.colorFrom: root.isDarkMode ? "#2A2A2A" : "#E8E8E8"
                    divider.colorTo:   root.isDarkMode ? "#1C1C1C" : "#FFFFFF"
                    onClicked: {
                        libraryPage.addDialogVisible = false
                        libraryPage._importStatus    = ""
                ...
                ListItem {
                    ...
                    onClicked: {
                        libraryPage.addDialogVisible = false
                        libraryPage.pdfSorryVisible  = true
                    }
                    Row {
                        anchors { verticalCenter: parent.verticalCenter; left: parent.left }
                        spacing: units.gu(1.8)
                        Icon {
                            width: units.gu(3); height: units.gu(3)
                            name: "document-open"; color: "#CC4444"
                            anchors.verticalCenter: parent.verticalCenter
                        }
                        Label {
                            text: "+ PDF File"
    ...
    // ── PDF "coming soon" dialog ───────────────────────────────────────────────
    Rectangle {
        id: pdfSorryOverlay
        anchors.fill: parent
        color: "#88000000"
        visible: libraryPage.pdfSorryVisible
        z: 161
        MouseArea { anchors.fill: parent; onClicked: libraryPage.pdfSorryVisible = false }

        Rectangle {
            anchors.centerIn: parent
            width: parent.width - units.gu(8)
            height: pdfSorryCol.implicitHeight + units.gu(6)
            color: root.isDarkMode ? "#1C1C1C" : "#FFFFFF"
            radius: units.dp(14)
            MouseArea { anchors.fill: parent; onClicked: {} }
            ...
            Column {
                ...
                Label {
                    anchors.horizontalCenter: parent.horizontalCenter
                    width: parent.width
                    text: "Sorry!  We're still working on this feature."
                    font.pixelSize: units.gu(2.0); font.weight: Font.Light
                ...
                Rectangle {
                    ...
                    MouseArea {
                        anchors.fill: parent
                        onClicked: libraryPage.pdfSorryVisible = false
                    }
                }
            }
        }
    ...
    // ── Book long-press context menu ──────────────────────────────────────────
    // Same slide-up style as the "Add to library" sheet. Three actions:
    //   ▶ Read           — open the EPUB reader directly
    //   + Add to List    — open the Reading List dialog
    //   ✕ Delete         — remove the book from the library immediately
    Rectangle {
        id: bookMenuOverlay
        anchors.fill: parent
        color: "#88000000"
        opacity: libraryPage.bookMenuVisible ? 1.0 : 0.0
        Behavior on opacity { NumberAnimation { duration: 200 } }
        visible: true   // kept alive so the slide-out animation completes
        z: 163

        MouseArea {
            anchors.fill: parent
            enabled: libraryPage.bookMenuVisible
            onClicked: libraryPage.bookMenuVisible = false
        }

        Rectangle {
            id: bookMenuSheet
            anchors { left: parent.left; right: parent.right }
            height: bookMenuCol.implicitHeight + units.gu(4.5)
            y: libraryPage.bookMenuVisible
               ? parent.height - height - units.gu(7)
               : parent.height + units.gu(1)
            Behavior on y { NumberAnimation { duration: 280; easing.type: Easing.OutCubic } }
            color: root.isDarkMode ? "#1C1C1C" : "#FFFFFF"
            ...
            Column {
                ...
                // Book title — small header label inside the sheet
                Label {
                    anchors { left: parent.left; right: parent.right }
                    text: libraryPage._longPressBook
                          ? (libraryPage._longPressBook.title || "") : ""
                    font.pixelSize: units.gu(1.9); font.weight: Font.Light
                    color: root.isDarkMode ? "#888888" : "#AAAAAA"
                ...
                // ▶ Read
                ListItem {
                    height: units.gu(7.5); color: "transparent"
                    divider.colorFrom: root.isDarkMode ? "#2A2A2A" : "#E8E8E8"
                    divider.colorTo:   root.isDarkMode ? "#1C1C1C" : "#FFFFFF"
                    onClicked: {
                        libraryPage.bookMenuVisible = false
                        if (libraryPage._longPressBook && root && root.openReader)
                            root.openReader(libraryPage._longPressBook)
                    }
                    Row {
                        anchors { verticalCenter: parent.verticalCenter; left: parent.left }
                        spacing: units.gu(1.8)
                        Icon {
                            width: units.gu(3); height: units.gu(3)
                            name: "media-playback-start"; color: "#4CAF50"
                            anchors.verticalCenter: parent.verticalCenter
                        }
                        Label {
                            text: "Read"
                ...
                ListItem {
                    ...
                    onClicked: {
                        libraryPage.bookMenuVisible = false
                        root.showRlDialog(libraryPage._longPressBook)
                    }
                    Row {
                        anchors { verticalCenter: parent.verticalCenter; left: parent.left }
                        spacing: units.gu(1.8)
                ...
                ListItem {
                    ...
                    onClicked: {
                        libraryPage.bookMenuVisible = false
                        if (libraryPage._longPressBook) {
                            Library.init()
                            Library.removeBook(libraryPage._longPressBook.id)
                            libraryPage.refresh()
                        }
                    }
                    Row {
    ...
    // ── Filter / Sort sheet ────────────────────────────────────────────────────
    // Slides up from the bottom. Shows universal sort options and user reading
    // lists. The header lists icon turns green when a filter is active.
    Rectangle {
        id: filterMenuOverlay
        anchors.fill: parent
        color: "#88000000"
        opacity: libraryPage.filterMenuVisible ? 1.0 : 0.0
        ...
        MouseArea {
            anchors.fill: parent
            enabled: libraryPage.filterMenuVisible
            onClicked: libraryPage.filterMenuVisible = false
        }

        Rectangle {
            id: filterSheet
            anchors { left: parent.left; right: parent.right }
            y: libraryPage.filterMenuVisible
               ? parent.height - filterSheet.height - units.gu(8)
               : parent.height
            Behavior on y { NumberAnimation { duration: 280; easing.type: Easing.OutCubic } }
            color: root.isDarkMode ? "#1C1C1C" : "#FFFFFF"
            radius: units.dp(14)
            height: filterSheetCol.implicitHeight + units.gu(3)
            MouseArea { anchors.fill: parent; onClicked: {} }
            ...
            Column {
                ...
                Item {
                    ...
                    Rectangle {
                        anchors.centerIn: parent
                        width: units.gu(5); height: units.dp(4)
                        radius: units.dp(2)
                        color: root.isDarkMode ? "#444444" : "#CCCCCC"
                    }
                }
                ...
                Item {
                    ...
                    property bool isActive: libraryPage.currentFilter === "all"
                    Row {
                        anchors { verticalCenter: parent.verticalCenter; left: parent.left; leftMargin: units.gu(2.5) }
                        spacing: units.gu(1.8)
                        Icon {
                            width: units.gu(3); height: units.gu(3)
                            name: "history"
                            color: parent.parent.isActive ? "#4CAF50" : "#8B5A32"
                            anchors.verticalCenter: parent.verticalCenter
                        }
                        Label {
                            text: "Recently added"
                            font.pixelSize: units.gu(2.0)
                            font.weight: parent.parent.isActive ? Font.Medium : Font.Light
                            color: parent.parent.isActive ? "#4CAF50"
                                   : (root.isDarkMode ? "#DDDDDD" : "#333333")
                            anchors.verticalCenter: parent.verticalCenter
                        }
                    }
                    Label {
                        anchors { right: parent.right; rightMargin: units.gu(7.5); verticalCenter: parent.verticalCenter }
                        text: libraryPage._allBooks.length + (libraryPage._allBooks.length === 1 ? " book" : " books")
                        font.pixelSize: units.gu(1.6)
                        color: root.isDarkMode ? "#666666" : "#AAAAAA"
                    }
                    Icon {
                        anchors { right: parent.right; rightMargin: units.gu(2.5); verticalCenter: parent.verticalCenter }
                        width: units.gu(2.5); height: units.gu(2.5)
                        name: "toolkit_checkmark"
                        color: "#4CAF50"
                        visible: parent.isActive
                    }
                    Rectangle {
                        anchors { bottom: parent.bottom; left: parent.left; right: parent.right; leftMargin: units.gu(2.5) }
                        height: units.dp(1); color: root.isDarkMode ? "#2A2A2A" : "#EEEEEE"
                    }
                    MouseArea {
                        anchors.fill: parent
                        onClicked: {
                            libraryPage.currentFilter = "all"
                            libraryPage.filterMenuVisible = false
                            libraryPage.refresh()
                        }
                    }
                }
                ...
                Item {
                    width: parent.width; height: units.gu(7)
                    property bool isActive: libraryPage.currentFilter === "unread"
                    property int  cnt: {
                        var n = 0
                        for (var i = 0; i < libraryPage._allBooks.length; i++)
                            if (libraryPage._allBooks[i].read_percent === 0 && !libraryPage._allBooks[i].is_finished) n++
                        return n
                    }
                    Row {
                        anchors { verticalCenter: parent.verticalCenter; left: parent.left; leftMargin: units.gu(2.5) }
                    ...
                    Label {
                        anchors { right: parent.right; rightMargin: units.gu(7.5); verticalCenter: parent.verticalCenter }
                        text: parent.cnt + (parent.cnt === 1 ? " book" : " books")
                        font.pixelSize: units.gu(1.6)
                        color: root.isDarkMode ? "#666666" : "#AAAAAA"
                    }
                    Icon {
                    ...
                    MouseArea {
                        anchors.fill: parent
                        onClicked: {
                            libraryPage.currentFilter = "unread"
                            libraryPage.filterMenuVisible = false
                            libraryPage.refresh()
                        }
                    }
                ...
                Item {
                    width: parent.width; height: units.gu(7)
                    property bool isActive: libraryPage.currentFilter === "in_progress"
                    property int  cnt: {
                        var n = 0
                        for (var i = 0; i < libraryPage._allBooks.length; i++)
                            if (libraryPage._allBooks[i].read_percent > 0 && !libraryPage._allBooks[i].is_finished) n++
                    ...
                    MouseArea {
                        anchors.fill: parent
                        onClicked: {
                            libraryPage.currentFilter = "in_progress"
                            libraryPage.filterMenuVisible = false
                            libraryPage.refresh()
                        }
                    }
                ...
                Item {
                    width: parent.width; height: units.gu(7)
                    property bool isActive: libraryPage.currentFilter === "list-starred"
                    property int  cnt: {
                        var n = 0
                        for (var j = 0; j < libraryPage._filterLists.length; j++)
                            if (libraryPage._filterLists[j].id === "list-starred")
                                { n = libraryPage._filterLists[j].book_ids.length; break }
                        return n
                    }
                    Row {
                        anchors { verticalCenter: parent.verticalCenter; left: parent.left; leftMargin: units.gu(2.5) }
                    ...
                    MouseArea {
                        anchors.fill: parent
                        onClicked: {
                            libraryPage.currentFilter = "list-starred"
                            libraryPage.filterMenuVisible = false
                            libraryPage.refresh()
                        }
                    }
                ...
                // ── User reading lists (excluding Starred) ────────────────────
                Repeater {
                    model: {
                        var out = []
                        for (var i = 0; i < libraryPage._filterLists.length; i++)
                            if (libraryPage._filterLists[i].id !== "list-starred")
                                out.push(libraryPage._filterLists[i])
                        return out
                    }
                    delegate: Item {
                        width: filterSheetCol.width; height: units.gu(7)
                        property bool isActive: libraryPage.currentFilter === modelData.id
                        // Divider above first user list
                        Rectangle {
                            anchors { top: parent.top; left: parent.left; right: parent.right; leftMargin: units.gu(2.5) }
                            height: units.dp(1)
                        ...
                        Row {
                            ...
                            Label {
                                ...
                                anchors.verticalCenter: parent.verticalCenter
                                elide: Text.ElideRight
                                width: filterSheetCol.width - units.gu(16)
                            }
                        }
                        Label {
                            anchors { right: parent.right; rightMargin: units.gu(7.5); verticalCenter: parent.verticalCenter }
                            text: modelData.book_ids.length + (modelData.book_ids.length === 1 ? " book" : " books")
                            font.pixelSize: units.gu(1.6)
                            color: root.isDarkMode ? "#666666" : "#AAAAAA"
                        }
                        Icon {
                        ...
                        MouseArea {
                            anchors.fill: parent
                            onClicked: {
                                libraryPage.currentFilter = modelData.id
                                libraryPage.filterMenuVisible = false
                                libraryPage.refresh()
                            }
                        }
