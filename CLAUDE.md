...
`start()` sends `pyotherside.send('streamer_ready', port, manifestUrl)` once the TCP poll succeeds; `StreamerLauncher.qml`'s `ready(port, manifestUrl)` signal carries both through to `ReaderPage._streamerPort` / `_streamerManifestUrl`. As of 2026-09-23 the streamer launches and serves correctly on-device but the WebView still loads `reader.html` (epub.js) — wiring the WebView to the manifest URL is Stage 6 / Step 7, gated on Stage 5 (bundling a Readium Web JS shell, not yet done).
